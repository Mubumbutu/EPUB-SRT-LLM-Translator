...
class _ReaderWebPage(QWebEnginePage):
    ...
    def acceptNavigationRequest(self, url, nav_type, is_main_frame):
        ...
        if scheme and scheme not in ('', 'about', 'data', 'file', 'qrc', 'epub-preview'):
            return False
        return True
...
class GenericReaderWindow(QMainWindow):
    """
    Full-screen reader window for FB2, DOCX, Mobi, PDF formats.
    Works identically to EPUBReaderWindow but uses format-specific engines.
    refresh_chapter() does a full re-render instead of JS injection.
    """

    closed = pyqtSignal()

    def __init__(self, file_type: str, source, paragraphs, selected_para,
                 dark_mode: bool = False, parent=None):
        super().__init__(parent)
        self.setWindowTitle(f"{file_type.upper()} Reader")
        self.setAttribute(Qt.WidgetAttribute.WA_DeleteOnClose, False)
    ...
    def _load_chapter(self, item_href: str, representative_para: dict):
        ...
        try:
            ...
            if ft == 'fb2':
                ...
                html_str = engine.generate_preview_html(
                    self._source, self._paragraphs, representative_para,
                    show_original_ids=None, dark_mode=self._dark_mode,
                    reader_mode=True, sepia_mode=self._sepia_mode,
                )
            elif ft == 'docx':
                ...
                html_str = '<html><body><p>Unsupported format.</p></body></html>'
        except Exception as exc:
            logger.error(f'[GenericReader] load failed: {exc}', exc_info=True)
            self._update_chapter_label()
            self._update_nav_buttons()
            self._update_status()
            return
        ...
        self._web.setHtml(html_str)
        self._update_chapter_label()
        self._update_nav_buttons()
        self._update_status()

    def refresh_chapter(self, paragraphs, selected_para, dark_mode: bool = None):
        self._paragraphs = paragraphs
        if dark_mode is not None and dark_mode != self._dark_mode:
            self._dark_mode = dark_mode
            self._btn_dark.setChecked(dark_mode)
            self._btn_dark.setText("☀ Light" if dark_mode else "🌙 Dark")
        ft = self._file_type
        try:
            if ft == 'fb2':
                js = FB2PreviewEngine().generate_refresh_js(
                    self._source, paragraphs, self._current_href
                )
            elif ft == 'docx':
                js = DOCXPreviewEngine().generate_refresh_js(
                    self._source, paragraphs, self._current_href
                )
            elif ft == 'mobi':
                js = MobiPreviewEngine().generate_refresh_js(
                    self._source, paragraphs, self._current_href
                )
            elif ft == 'pdf':
                js = PDFPreviewEngine().generate_refresh_js(
                    self._source, paragraphs, self._current_href
                )
            else:
                js = ''
            ...
            logger.warning(f'[GenericReader] refresh JS failed: {exc}', exc_info=True)
            self._reload_current()
        self._update_status()

    def navigate_to_para(self, paragraphs, para_index: int, dark_mode: bool = None):
        self._paragraphs = paragraphs
        if dark_mode is not None:
            self._dark_mode = dark_mode
        if para_index < 0 or para_index >= len(paragraphs):
...
class FB2PreviewEngine:
    """Preview engine for FB2 files."""
    ...
    def generate_preview_html(
        ...
        try:
            return self._build_html(fb2_path, paragraphs, selected_para,
                                    show_original_ids, dark_mode, reader_mode, sepia_mode)
        except Exception as exc:
            logger.error(f'[FB2Preview] generation failed: {exc}', exc_info=True)
            return (
                '<html><body style="background:#1a1a1a;padding:24px;">'
                f'<p style="color:#ff6b6b;font-family:monospace;font-size:13px;">'
                f'FB2 Preview error: {exc}</p></body></html>'
            )

    def _build_html(self, fb2_path, paragraphs, selected_para, show_original_ids,
                    dark_mode, reader_mode=False, sepia_mode=False):
        from lxml import etree as _etree
        import html as _html_mod

        _show_original = show_original_ids if show_original_ids is not None else set()
        item_href = selected_para.get('item_href', 'chapter_0')

        # Parse chapter index from item_href like 'chapter_3'
        try:
            chapter_idx = int(item_href.split('_')[-1])
        except (ValueError, IndexError):
            chapter_idx = 0

        # Parse FB2 (obsługa skompresowanych .fb2.gz)
        with open(fb2_path, 'rb') as f:
            fb2_bytes = f.read()
        if fb2_bytes[:2] == b'\x1f\x8b':
            import gzip
            fb2_bytes = gzip.decompress(fb2_bytes)
        tree = _etree.fromstring(fb2_bytes)
        ns = self.FB2_NS

        # Find all body elements, skip notes/footnotes/comments
        bodies = tree.findall(f'{{{ns}}}body')
        main_body = None
        for b in bodies:
            name = b.get('name', '')
            if name.lower() in ('notes', 'footnotes', 'comments'):
                continue
            main_body = b
            break
        ...
        if main_body is None:
            return '<html><body><p>No body found in FB2.</p></body></html>'

        sections = main_body.findall(f'{{{ns}}}section')
        if chapter_idx < len(sections):
            section = sections[chapter_idx]
        elif sections:
            section = sections[0]
        else:
            return '<html><body><p>No sections found in FB2.</p></body></html>'

        # Collect paragraphs for this chapter
        chapter_paras = [p for p in paragraphs if p.get('item_href') == item_href]
        ...
        for para in chapter_paras:
            ...
            if (para.get('is_translated') and para.get('translated_text', '').strip()
                    and pid not in _show_original):
                trans_map[pid] = para.get('translated_text', '')

        # Build HTML body elements
        HEADING_TAGS = {f'{{{ns}}}title', f'{{{ns}}}subtitle'}
        BLOCK_TAGS = {f'{{{ns}}}p', f'{{{ns}}}epigraph', f'{{{ns}}}poem',
                      f'{{{ns}}}stanza', f'{{{ns}}}v', f'{{{ns}}}empty-line',
                      f'{{{ns}}}cite', f'{{{ns}}}text-author', f'{{{ns}}}date'}

        def elem_text(elem):
            """Get full text content of an element."""
            parts = []
            if elem.text:
                parts.append(_html_mod.escape(elem.text))
            for child in elem:
                local = child.tag.split('}')[-1] if '}' in child.tag else child.tag
                if local == 'emphasis':
                    inner = _html_mod.escape(child.text or '') if child.text else ''
                    ...
                    inner = _html_mod.escape(child.text or '') if child.text else ''
                    parts.append(f'<a href="#">{inner}</a>')
                else:
                    if child.text:
                        parts.append(_html_mod.escape(child.text))
                if child.tail:
                    parts.append(_html_mod.escape(child.tail))
            return ''.join(parts)

        # Iterate section children in order, match with chapter_paras
        para_iter = iter(chapter_paras)
        html_parts = []
        ...
        def process_elem(elem, depth=0):
            ...
            if local in ('title', 'subtitle'):
                try:
                    para = next(para_iter)
                except StopIteration:
                    para = None
                pid = para['id'] if para else ''
                selected_attr = ' data-epub-preview-selected="1"' if (not reader_mode and para and pid == selected_para.get('id')) else ''
                orig_attr = ' data-epub-preview-show-original="1"' if para and pid in _show_original else ''
                text = trans_map.get(pid) if pid else None
                if text is None:
                    text_parts = []
                    for child in elem:
                        if child.tag == f'{{{ns}}}p' or child.tag.split('}')[-1] == 'p':
                            text_parts.append(elem_text(child))
                    text = ' '.join(text_parts) or _html_mod.escape(elem.text or '')
                else:
                    text = _html_mod.escape(text)
                id_attr = f' id="{pid}"' if pid else ''
                tag_name = 'h2' if depth == 0 else 'h3'
                html_parts.append(f'<{tag_name}{id_attr}{selected_attr}{orig_attr}>{text}</{tag_name}>')
                ...
                if text is None:
                    ...
                    text = _html_mod.escape(text)
                id_attr = f' id="{pid}"' if pid else ''
                html_parts.append(f'<blockquote{id_attr}{selected_attr}{orig_attr}>{text}</blockquote>')
                ...
                if text is None:
                    ...
                    text = _html_mod.escape(text)
                id_attr = f' id="{pid}"' if pid else ''
                html_parts.append(f'<p{id_attr}{selected_attr}{orig_attr}>{text}</p>')
                ...
                for child in elem:
                    ...
                    if child_local in ('p', 'title', 'subtitle', 'epigraph', 'empty-line', 'section'):
                        process_elem(child, depth)

        for child in section:
            process_elem(child)

        if reader_mode:
            base_css = _READER_PAGE_CSS
        else:
            base_css = """
body { font-family: Georgia, serif; font-size: 16px; line-height: 1.7;
       margin: 0; padding: 24px 32px; max-width: 820px; }
h2, h3 { font-size: 1.2em; font-weight: bold; margin: 1.2em 0 0.5em; }
p { margin: 0.6em 0; }
blockquote { margin: 1em 2em; font-style: italic; border-left: 3px solid #888; padding-left: 0.8em; }
"""

        css_blocks = [base_css, _HIGHLIGHT_CSS]
        if not reader_mode:
            css_blocks.append(_SELECTED_HIGHLIGHT_CSS)
            css_blocks.append(_PREVIEW_SCROLLBAR_CSS)
        else:
            css_blocks.append(_NO_SCROLLBAR_CSS)
        if dark_mode:
            css_blocks.append(_DARK_MODE_CSS)
        if sepia_mode:
            css_blocks.append(_READER_PAGE_SEPIA_CSS)

        css = '\n'.join(css_blocks)
        body_html = '\n'.join(html_parts)
        interaction_js = _DRAG_SCROLL_JS if reader_mode else _INTERACTION_JS

        return (
            '<!DOCTYPE html>\n<html>\n<head>\n<meta charset="utf-8">\n'
            f'<style>{css}</style>\n</head>\n<body>\n'
            f'{body_html}\n'
            f'<script>{_SCROLL_TO_SEL_JS}</script>\n'
            f'<script>{interaction_js}</script>\n'
            '</body>\n</html>'
        )


    def generate_refresh_js(self, source, paragraphs: list, item_href: str,
                             show_original_ids=None) -> str:
        import html as _html_mod
        _show_original = show_original_ids or set()
        js_parts = []
        for para in paragraphs:
            ...
            if not elem_id or elem_id in _show_original:
                continue
            text = _html_mod.escape(para['translated_text'])
            safe_id = elem_id.replace('\\', '\\\\').replace('"', '\\"')
            escaped = text.replace('\\', '\\\\').replace('`', '\\`').replace('${', '\\${')
            js_parts.append(
                f'(function(){{var e=document.getElementById("{safe_id}");'
                f'if(e){{e.innerHTML=`{escaped}`;}}}})(  );'.replace('  ', '')
            )
        return '\n'.join(js_parts)
...
class DOCXPreviewEngine:
    """Preview engine for DOCX files."""
    ...
    def generate_preview_html(
        ...
        try:
            return self._build_html(docx_path, paragraphs, selected_para,
                                    show_original_ids, dark_mode, reader_mode, sepia_mode)
        except Exception as exc:
            logger.error(f'[DOCXPreview] generation failed: {exc}', exc_info=True)
            return (
                '<html><body style="background:#1a1a1a;padding:24px;">'
                f'<p style="color:#ff6b6b;font-family:monospace;font-size:13px;">'
                f'DOCX Preview error: {exc}</p></body></html>'
            )

    def _build_html(self, docx_path, paragraphs, selected_para, show_original_ids,
                    dark_mode, reader_mode=False, sepia_mode=False):
        import html as _html_mod
        ...
        chapter_index = 0
        doc_paras_for_chapter = []
        for doc_para in doc.paragraphs:
            if not doc_para.text.strip():
                continue
            style_name = (doc_para.style.name or '').lower()
            if style_name.startswith('heading'):
                chapter_index += 1
            if f'chapter_{chapter_index}' == item_href:
                doc_paras_for_chapter.append(doc_para)

        html_parts = []
        para_iter = iter(chapter_paras)

        for doc_para in doc_paras_for_chapter:
            try:
                meta = next(para_iter)
            except StopIteration:
                break
            ...
            selected_attr = ' data-epub-preview-selected="1"' if (not reader_mode and pid == selected_para.get('id')) else ''
            orig_attr = ' data-epub-preview-show-original="1"' if pid in _show_original else ''
            id_attr = f' id="{pid}"' if pid else ''

            if pid in trans_map:
                text = _html_mod.escape(trans_map[pid])
            else:
                run_parts = []
                for run in doc_para.runs:
                    rt = _html_mod.escape(run.text or '')
                    if run.bold and run.italic:
                        rt = f'<strong><em>{rt}</em></strong>'
                    elif run.bold:
                        rt = f'<strong>{rt}</strong>'
                    elif run.italic:
                        rt = f'<em>{rt}</em>'
                    run_parts.append(rt)
                text = ''.join(run_parts) or _html_mod.escape(doc_para.text)

            if is_heading:
                html_parts.append(f'<h2{id_attr} class="heading"{selected_attr}{orig_attr}>{text}</h2>')
            else:
                html_parts.append(f'<p{id_attr} class="paragraph"{selected_attr}{orig_attr}>{text}</p>')

        if reader_mode:
            base_css = _READER_PAGE_CSS
        else:
            base_css = """
body { font-family: Arial, sans-serif; font-size: 15px; line-height: 1.7;
       margin: 0; padding: 24px 32px; max-width: 820px; }
h2 { font-size: 1.2em; font-weight: bold; margin: 1.2em 0 0.5em; }
p.paragraph { margin: 0.6em 0; }
"""
...
class MobiPreviewEngine:
    """Preview engine for Mobi/AZW3 files."""
    ...
    def generate_preview_html(
        ...
        try:
            return self._build_html(kindle_processor, paragraphs, selected_para,
                                    show_original_ids, dark_mode, reader_mode, sepia_mode)
        except Exception as exc:
            logger.error(f'[MobiPreview] generation failed: {exc}', exc_info=True)
            return (
                '<html><body style="background:#1a1a1a;padding:24px;">'
                f'<p style="color:#ff6b6b;font-family:monospace;font-size:13px;">'
                f'Mobi Preview error: {exc}</p></body></html>'
            )

    def _build_html(self, kindle_processor, paragraphs, selected_para, show_original_ids,
                    dark_mode, reader_mode=False, sepia_mode=False):
        tempdir = getattr(kindle_processor, '_preview_tempdir', None)
        if tempdir is None:
            return (
                '<html><body style="background:#1a1a1a;padding:24px;">'
                '<p style="color:#888;font-family:sans-serif;font-size:13px;">'
                'Mobi preview: temporary directory not available. '
                'Please reload the file.</p></body></html>'
            )
        ...
        for root_dir, dirs, files in os.walk(tempdir):
            for fname in files:
                fl = fname.lower()
                if fl.endswith('.epub') and epub_path is None:
                    epub_path = os.path.join(root_dir, fname)
                elif fl.endswith('.html') and html_path is None:
                    html_path = os.path.join(root_dir, fname)

        if epub_path:
            try:
                import epub_utils
                book = epub_utils.read_epub(epub_path)
                engine = EPUBPreviewEngine()
                if reader_mode:
                    css_extra = _READER_PAGE_CSS
                    if sepia_mode:
                        css_extra += _READER_PAGE_SEPIA_CSS
                        ...
                        css_extra += _READER_PAGE_DARK_CSS
                    return engine.generate_preview_html(
                        book, paragraphs, selected_para,
                        show_original_ids=show_original_ids,
                        dark_mode=dark_mode,
                        reader_css=css_extra,
                        scroll_to_selected=not reader_mode,
                        reader_mode=True,
                        skip_mismatch=True,
                    )
                else:
                ...
                logger.error(f'[MobiPreview] EPUB delegation failed: {exc}', exc_info=True)
                return (
                    '<html><body style="background:#1a1a1a;padding:24px;">'
                    f'<p style="color:#ff6b6b;font-family:monospace;font-size:13px;">'
                    f'Mobi/EPUB Preview error: {exc}</p></body></html>'
                )

        elif html_path:
            return self._build_html_from_file(html_path, paragraphs, selected_para,
                                              show_original_ids, dark_mode, reader_mode, sepia_mode)
            ...
            return (
                '<html><body style="background:#1a1a1a;padding:24px;">'
                '<p style="color:#888;font-family:sans-serif;font-size:13px;">'
                'Mobi preview: no extractable content found.</p></body></html>'
            )

    def _build_html_from_file(self, html_path, paragraphs, selected_para, show_original_ids,
                               dark_mode, reader_mode=False, sepia_mode=False):
        import html as _html_mod
        from lxml import etree as _etree

        _show_original = show_original_ids if show_original_ids is not None else set()
        item_href = selected_para.get('item_href', '0')
        chapter_paras = [p for p in paragraphs if p.get('item_href') == item_href]
        ...
        with open(html_path, 'rb') as f:
            content = f.read()
        try:
            tree = _etree.fromstring(content, parser=_etree.HTMLParser())
        except Exception:
            return '<html><body><p>Could not parse Mobi HTML.</p></body></html>'

        body = tree.find('.//body')
        if body is None:
            return '<html><body><p>No body in Mobi HTML.</p></body></html>'
        ...
        for elem in body.iter():
            ...
            if tag not in HEADING_TAGS and tag != 'p':
                continue
            text_raw = (_etree.tostring(elem, encoding='unicode', method='text') or '').strip()
            if not text_raw:
                continue
            try:
                meta = next(para_iter)
            ...
            orig_attr = ' data-epub-preview-show-original="1"' if pid in _show_original else ''
            id_attr = f' id="{pid}"' if pid else ''
            text = _html_mod.escape(trans_map.get(pid, text_raw))
            ht = 'h2' if tag in HEADING_TAGS else 'p'
            html_parts.append(f'<{ht}{id_attr}{selected_attr}{orig_attr}>{text}</{ht}>')
...
class PDFPreviewEngine:
    """Preview engine for PDF files — renders a page as an image with overlay divs."""
    ...
    def generate_preview_html(
        ...
        try:
            return self._build_html(pdf_path, paragraphs, selected_para,
                                    show_original_ids, dark_mode, reader_mode, sepia_mode)
        except Exception as exc:
            logger.error(f'[PDFPreview] generation failed: {exc}', exc_info=True)
            return (
                '<html><body style="background:#1a1a1a;padding:24px;">'
                f'<p style="color:#ff6b6b;font-family:monospace;font-size:13px;">'
                f'PDF Preview error: {exc}</p></body></html>'
            )

    def _build_html(self, pdf_path, paragraphs, selected_para, show_original_ids,
                    dark_mode, reader_mode=False, sepia_mode=False):
        import fitz
        import base64
        import html as _html_mod
        ...
        _show_original = show_original_ids if show_original_ids is not None else set()
        page_number = int(selected_para.get('item_href', '0'))
        ...
        page_paras = [p for p in paragraphs if p.get('item_href') == str(page_number)]

        if sepia_mode:
            bg_color = '#e8dcc8'
            frag_text_bg = 'rgba(232,220,200,0.92)'
            frag_text_color = '#2c1a0e'
        elif dark_mode:
            bg_color = '#1a1a1a'
            frag_text_bg = 'rgba(26,26,26,0.92)'
            frag_text_color = '#cccccc'
        else:
            bg_color = '#f0f0f0'
            frag_text_bg = 'rgba(240,240,240,0.92)'
            frag_text_color = '#222222'

        frag_divs = []
        for para in page_paras:
            pid = para.get('id', '')
            bbox = para.get('bbox', [0, 0, 10, 10])
            x0, y0, x1, y1 = [c * scale for c in bbox]
            width = max(x1 - x0, 10)
            height = max(y1 - y0, 10)
            font_size = para.get('span_metadata', {}).get('font_size', 12) * scale * 0.9

            css_classes = 'frag'
            attrs = ''
            if not reader_mode and pid == selected_para.get('id'):
                attrs += ' data-epub-preview-selected="1"'
            if pid in _show_original:
                attrs += ' data-epub-preview-show-original="1"'

            translated_text = ''
            if (para.get('is_translated') and para.get('translated_text', '').strip()
                    and pid not in _show_original):
                css_classes += ' translated'
                translated_text = _html_mod.escape(para['translated_text'])

            frag_divs.append(
                f'<div id="{pid}" class="{css_classes}"'
                f' style="left:{x0:.1f}px;top:{y0:.1f}px;width:{width:.1f}px;height:{height:.1f}px"'
                f'{attrs}>'
                f'<div class="frag-text" style="font-size:{font_size:.1f}px">{translated_text}</div>'
                f'</div>'
            )

        frags_html = '\n'.join(frag_divs)

        sepia_img_filter = 'filter: sepia(0.55) contrast(0.92) brightness(1.04);' if sepia_mode else ''
        selected_css = '' if reader_mode else """
[data-epub-preview-selected] {
    outline: 2px solid #4a90d9 !important;
    background: rgba(74,144,217,0.12) !important;
}
"""
        scrollbar_css = _NO_SCROLLBAR_CSS if reader_mode else _PREVIEW_SCROLLBAR_CSS

        css = f"""
body {{ margin: 0; padding: 16px; background: {bg_color}; }}
.page-container {{
    position: relative;
    width: {page_w}px;
    height: {page_h}px;
...
.frag {{
    position: absolute;
    cursor: {'default' if reader_mode else 'pointer'};
    border-radius: 2px;
    box-sizing: border-box;
}}
{'' if reader_mode else '''.frag:hover {{
    background: rgba(74,144,217,0.08);
    outline: 1px dashed rgba(74,144,217,0.4);
}}'''}
{{selected_css}}[data-epub-preview-show-original] {{
    outline: 2px dashed #e09c2a !important;
    background: rgba(224,156,42,0.10) !important;
}}
.frag-text {{
    position: absolute; top: 0; left: 0; right: 0;
    background: {frag_text_bg};
    color: {frag_text_color};
    padding: 2px 4px;
    line-height: 1.4;
    border-radius: 2px;
    display: none;
}}
.frag.translated .frag-text {{ display: block; }}
{{scrollbar_css}}
""".replace('{selected_css}', selected_css).replace('{scrollbar_css}', scrollbar_css)
        ...
        return (
            ...
            f'<img class="page-bg" draggable="false" src="data:image/png;base64,{img_b64}">\n'
            f'{frags_html}\n'
            f'</div>\n'
            f'<script>{_SCROLL_TO_SEL_JS}</script>\n'
            f'<script>{interaction_js}</script>\n'
    ...
    def generate_refresh_js(self, pdf_path: str, paragraphs: list, item_href: str,
                             show_original_ids=None) -> str:
        import html as _html_mod
        _show_original = show_original_ids or set()
        page_number_str = item_href
        js_parts = []
        for para in paragraphs:
            if para.get('item_href') != page_number_str:
                continue
            if not para.get('is_translated') or not para.get('translated_text', '').strip():
                continue
            elem_id = para.get('id')
            ...
            js_parts.append(
                f'(function(){{'
                f'var e=document.getElementById("{safe_id}");'
                f'if(e){{'
                f'e.classList.add("translated");'
                f'var t=e.querySelector(".frag-text");'
                f'if(t){{t.innerHTML=`{escaped}`;}}}}}})(  );'.replace('  ', '')
            )
        return '\n'.join(js_parts)


# backward-compat alias
PreviewToolbar = EPUBPreviewToolbar
